...
const projects = [
  {
    ...
    details: [
      ...
      'Tech stack: Next.js 16, React 19, Tailwind CSS, Three.js, and GSAP, deployed on Vercel.',
    ],
    gallery: [],
  },
  {
    ...
    deployed: true,
    details: [
      'An AI content-automation platform that turns a daily news feed into review-ready LinkedIn and Instagram posts for a Dubai-based eCommerce media brand.',
      'A three-layer system: a FastAPI backend ingests 50+ RSS feeds and NewsData.io results, deduplicates them, and scores relevance with an LLM (anything below the 0.7 threshold is pruned). A Next.js dashboard then generates post copy and image prompts with GPT-4o-mini and hands rendering off to n8n workflows, which produce single images or multi-slide carousel decks and upload them to AWS S3. Editors work a pending/approved/published queue; on approval the backend stitches carousel slides into a LinkedIn-ready PDF, and n8n drives the final publish, with email notifications on each transition.',
      'Tech stack: Next.js 15, React 19, TypeScript, and Tailwind CSS; FastAPI, SQLAlchemy, and Pydantic on the backend; PostgreSQL; OpenAI GPT-4o-mini; n8n automation; AWS S3 for image hosting; deployed on AWS Amplify and Docker.',
    ],
    gallery: [],
  },
  {
    ...
    description:
      'A browser-based console for running hiring-post scrapers across Reddit and Upwork, with live results, CSV/XLSX export, and a persisted run history.',
    image: null,
    github: 'https://github.com/AndrewRaj0510/Scraper-Web',
    visibility: 'private',
    deployed: true,
    details: [
      'A two-service web app that runs hiring-post scrapers from the browser, pulling job and gig leads from Reddit and Upwork into structured, exportable tables.',
      'A Next.js frontend offers a form per scraper and proxies requests to a FastAPI backend, which runs the Python scrapers: the Reddit scraper queries search.rss across a list of hiring phrases (throttled to stay under Reddit’s rate limit), dedupes, and floats priority-keyword matches to the top, while the Upwork scraper wraps an Apify actor. Every run is persisted to PostgreSQL and its XLSX and log artifacts are uploaded to S3, all browsable from a Run History page.',
      'Tech stack: Next.js (App Router) and React on the frontend; FastAPI and SQLAlchemy on the backend; PostgreSQL; AWS S3 for artifacts; an Apify actor for Upwork; containerized with Docker and deployable to AWS Amplify and App Runner.',
    ],
    gallery: [],
  },
  {
